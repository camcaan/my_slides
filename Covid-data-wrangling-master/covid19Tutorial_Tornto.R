...
UnitedKingdom <- country %>% filter(Country.Region=="United Kingdom")
...
# UK confirmed
ggplot(UnitedKingdom, aes(x=date, y=confirmed)) + geom_bar(stat="identity", width=0.1) +
  theme_classic() +
  labs(title = "Covid-19 Confirmed Cases in the UK", x= "Date", y= "Daily confirmed cases") +
  theme(plot.title = element_text(hjust = 0.5))
...
# World confirmed
ggplot(UnitedKingdom, aes(x=days, y=confirmed)) + geom_line() +
  theme_classic() +
